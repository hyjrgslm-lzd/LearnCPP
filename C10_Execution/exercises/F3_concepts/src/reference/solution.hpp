...
template <class R, class Sig> struct handles_one : std::false_type {};
template <class R, class... Args>
    struct handles_one<R, set_value_t(Args...)> : std::bool_constant <
                                                  requires(R &receiver, Args... args) {
  receiver.set_value(args...);
}>{};
template <class R, class Error>
    struct handles_one<R, set_error_t(Error)> : std::bool_constant <
                                                requires(R &receiver, Error error) {
  receiver.set_error(error);
}>{};
template <class R>
    struct handles_one<R, set_stopped_t()> : std::bool_constant < requires(R &receiver) {
  receiver.set_stopped();
}>{};
template <class R, class Sigs> struct handles_all;
template <class R, class... Sigs>
struct handles_all<R, completion_signatures<Sigs...>>
    : std::bool_constant<(handles_one<R, Sigs>::value && ...)> {};
template <class R, class Sigs>
concept my_receiver_of = my_receiver<R> && handles_all<R, Sigs>::value;
