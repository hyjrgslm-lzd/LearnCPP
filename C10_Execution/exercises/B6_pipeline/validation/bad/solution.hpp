...
inline Report run_pipeline(const std::vector<std::string> &input, std::size_t max_records,
  ...
  exec::static_thread_pool one_pool(1);
  auto sch = one_pool.get_scheduler();
  auto branch = [&](int id) {
    return ex::starts_on(sch, ex::just(std::move(buckets[id]), id) |
                                  ex::then([](std::vector<std::string> records, int branch_id) {
                                    return parse_batch(std::move(records), branch_id);
                                  }) |
                                  ex::continues_on(sch) | ex::then([&](Batch batch) {
                                    return enrich_batch(std::move(batch), hook);
                                  }));
  };
  auto graph =
      ex::when_all(branch(0), branch(1), branch(2)) | ex::continues_on(sch) |
