...
struct read_sender {
  ...
  template <class Receiver> struct op {
    ...
    struct shared_state {
      ...
      using request_guard = std::lock_guard<std::mutex>;
      ...
      void request_stop() noexcept {
        ...
        std::shared_ptr<c10_native::read_request> active;
        {
          request_guard guard{request_mutex};
          active = request;
        }
        if (active)
          active->cancel();
    ...
    void start() & noexcept {
      ...
      {
        typename shared_state::request_guard guard{state->request_mutex};
...
inline auto read_at(std::shared_ptr<native_file> file, std::uint64_t offset,
                    std::span<std::byte> buffer) {
  return read_sender{std::move(file), offset, buffer};
}
} // namespace c10_i1