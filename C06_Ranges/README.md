...
默认构建保持 **C++26 / MSVC `/std:c++latest`**。知识分别标明 C++20/23、C++26/C++29 草案及追溯缺陷修正；语言模式、标准库声明和实际运行能力分别验证。覆盖与迁移见[覆盖表](references/coverage.md)。
...
[B01成本实验](exercises/B01_cost/README.md)比较日志查询/收束与索引构建/查找。它们有相同契约下的对照，也明确标记独立整数键机制分支；先取得正确性与操作计数，再记录时间，不预设某个容器或Ranges更快。
