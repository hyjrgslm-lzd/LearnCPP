...
class my_transform_view : public std::ranges::view_interface<my_transform_view<V, F>> {
    ...
    public:
        ...
        reference operator*() const { if constexpr (std::copy_constructible<F>) { auto copy = *function_; return std::invoke(copy, *current_); } else { return std::invoke(*function_, *current_); } }
