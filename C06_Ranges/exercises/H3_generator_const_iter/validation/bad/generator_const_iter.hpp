...
public:
    struct promise_type {
        ...
        std::suspend_always yield_value(T&& value) { value_ = std::forward<T>(value); return {}; }
    };
    ...
    iterator begin() { if (!h_) return {}; h_.resume(); if (h_.promise().error_) std::rethrow_exception(h_.promise().error_); return iterator{h_}; }
