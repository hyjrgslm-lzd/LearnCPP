...
宏值以[标准索引](../../references/standards.md)和当前工具链实际声明为准。每个`.cpp`保留完整主体；条件不满足时只运行说明缺失的驱动，返回77由CTest标SKIP。条件满足后会真正编译、链接和运行主体，任何错误都是失败，不转换成SKIP。未实现分支不能宣称“已经编译了所有前沿代码”。

## Part与解析

两项C++29宏需要用同一份源码和同一工具链模式判断，不能把一次旧探测结果改写成当前能力结论。
