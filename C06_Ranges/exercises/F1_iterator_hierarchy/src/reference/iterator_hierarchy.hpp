...
struct ptr_iterator {
    ...
    using iterator_category = std::conditional_t<std::same_as<Tag, std::contiguous_iterator_tag>,
    ...
    friend ptr_iterator operator-(ptr_iterator it, difference_type n) requires std::derived_from<Tag, std::random_access_iterator_tag> { it -= n; return it; }
    friend difference_type operator-(ptr_iterator a, ptr_iterator b) requires std::derived_from<Tag, std::random_access_iterator_tag> { return a.p - b.p; }
...
struct range_of {
    using iterator = It;
    std::vector<int> data;
    It begin() { return It{data.data()}; }
    It end() { return It{data.data() + data.size()}; }
};
