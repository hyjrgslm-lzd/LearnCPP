...
template<class R>
concept has_any_adl_begin = requires(R& range) {
...
struct my_begin_fn {
    ...
    template<class R>
        requires detail::safe_range_object<R> && detail::has_any_adl_begin<std::remove_reference_t<R>>
    constexpr auto operator()(R&& range) const noexcept(noexcept(begin(range))) {
        return begin(range);
    }

    template<class R>
        requires detail::safe_range_object<R> &&
                 (!detail::has_any_adl_begin<std::remove_reference_t<R>>) &&
                 detail::has_member_begin<std::remove_reference_t<R>>
