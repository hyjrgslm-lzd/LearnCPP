...
inline std::vector<LogRecord> collect_records_ranges_once_impl(std::span<const std::string_view> lines,
    ...
    auto has_value = [](const std::optional<LogRecord>& record) {
        return record.has_value();
    };
    auto unwrap_by_value = [](std::optional<LogRecord> record) {
        return std::move(*record);
    };

    auto records = parsed | std::views::filter(has_value)
                | std::views::transform(unwrap_by_value);
