...
public:
    ...
    void reserve(std::size_t requested) {
        ...
        try {
            ...
            traits::deallocate(alloc_, next, requested);
            throw;
        }
    ...
    void push_back(T value) {
        ...
        capacity_ = requested;
        try {
            traits::construct(alloc_, data_ + old_size, append_source(value));
        } catch (...) {
            traits::deallocate(alloc_, next, requested);
            data_ = old_data;
            size_ = old_size;
        ...
        std::size_t built = 0;
        try {
            for (; built != old_size; ++built) {
                construct_relocated(data_ + built, old_data[built]);
            }
        } catch (...) {
            destroy_prefix(data_, built);
            traits::destroy(alloc_, data_ + old_size);
            traits::deallocate(alloc_, data_, capacity_);
            data_ = old_data;
            size_ = old_size;
        ...
        destroy_prefix(old_data, old_size);
        if (old_data != nullptr) {
            traits::deallocate(alloc_, old_data, old_capacity);
        }
        size_ = old_size + 1;
    }
