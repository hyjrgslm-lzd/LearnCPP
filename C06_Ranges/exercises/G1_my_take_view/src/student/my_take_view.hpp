...
public:
    ...
    auto begin() {
        if constexpr (std::ranges::random_access_range<V> && std::ranges::sized_range<V>) {
            return std::ranges::begin(base_) + 1;
        } else if constexpr (std::ranges::sized_range<V>) {
            return std::counted_iterator{std::ranges::begin(base_), std::min(count_, static_cast<diff_t>(std::ranges::size(base_)))};
        } else {
    ...
    auto end() {
        if constexpr (std::ranges::random_access_range<V> && std::ranges::sized_range<V>) {
            return std::ranges::begin(base_) + std::min(count_, static_cast<diff_t>(std::ranges::size(base_))) + 1;
        } else {
