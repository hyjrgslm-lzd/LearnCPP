...
从 Concurrency_Study/exercises 执行以下 PowerShell 命令；此题独占 build/l3-student，不与公共 preset 目录混用：
...
**D：写清证据边界。** [VALIDATION](../../topics/performance/VALIDATION.md) 保留了开发六组原始样本及解释，但这些样本处于集成构建未空闲环境，并非当前冻结源码的正式性能结论。正式报告应使用稳定源码的新构建及新输出目录；一次成功运行或 Reference 通过不能证明全部调度历史、真实 worker 数或峰值带宽。
