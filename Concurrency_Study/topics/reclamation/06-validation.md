...
```powershell
./Concurrency_Study/topics/reclamation/verify.ps1                    # 默认 Reference：6 个程序应返回 0
./Concurrency_Study/topics/reclamation/verify.ps1 -Mode Starter      # 四个空 main 应返回 1，且未启动 worker
./Concurrency_Study/topics/reclamation/verify.ps1 -Mode Student      # 四个补全 main 必须实际检查通过，返回 0
./Concurrency_Study/topics/reclamation/verify.ps1 -Asan              # 默认 Reference 的 ASan 验证
```
...
cl /nologo /std:c++23preview /EHsc /W4 /WX /utf-8 /O2 /DNDEBUG /IConcurrency_Study/exercises/include /FoConcurrency_Study/topics/reclamation/.build/I2_student.obj /FeConcurrency_Study/topics/reclamation/.build/I2_student.exe Concurrency_Study/exercises/I2_hazard_pointer/main.cpp
./Concurrency_Study/topics/reclamation/.build/I2_student.exe
$LASTEXITCODE # 空 starter 为 1；全部学生检查通过后才为 0
cl /nologo /std:c++23preview /EHsc /W4 /WX /utf-8 /O2 /DNDEBUG /IConcurrency_Study/exercises/include /FoConcurrency_Study/topics/reclamation/.build/I2_reference.obj /FeConcurrency_Study/topics/reclamation/.build/I2_reference.exe Concurrency_Study/exercises/I2_hazard_pointer/solution.cpp
./Concurrency_Study/topics/reclamation/.build/I2_reference.exe
```
...
cl /nologo /std:c++23preview /EHsc /W4 /WX /utf-8 /O2 /DNDEBUG ^
 /IConcurrency_Study/exercises/include ^
 /FoConcurrency_Study/topics/reclamation/reclamation_test.obj ^
 /FeConcurrency_Study/topics/reclamation/reclamation_test.exe ^
 Concurrency_Study/exercises/runtime_tests/reclamation_test.cpp
Concurrency_Study\topics\reclamation\reclamation_test.exe
```

把输入路径换成某题的 solution.cpp 或本目录 lifetime_reference.cpp，就得到对应独立程序；同时给输出选择不同文件名。GCC/Clang 的预期形式如下，但本批未在这两种工具链运行，不把命令示例标记为验证通过：

```sh
c++ -std=c++23 -O2 -DNDEBUG -pthread -IConcurrency_Study/exercises/include \
  Concurrency_Study/exercises/runtime_tests/reclamation_test.cpp \
  -o Concurrency_Study/topics/reclamation/reclamation_test
```
...
```powershell
./Concurrency_Study/topics/reclamation/.build/exercises_runtime_tests_reclamation_test.exe --failure-paths
./Concurrency_Study/topics/reclamation/.build-asan/exercises_runtime_tests_reclamation_test.exe --failure-paths
```
...
| 文件（相对 Concurrency_Study） | SHA-256 |
|---|---|
