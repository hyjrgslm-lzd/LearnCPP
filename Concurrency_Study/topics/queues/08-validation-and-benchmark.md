...
以下在 `Concurrency_Study/exercises` 下执行，假设已由公共 CMake 构建 benchmark、runtime 与各 Reference。可执行文件目录因总构建或叶构建而不同，实际使用相应路径。runner 本身为 [`tools/run_benchmarks.py`](../../exercises/tools/run_benchmarks.py)。
