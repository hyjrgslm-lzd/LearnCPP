...
inline std::shared_ptr<int> alias_value(const std::shared_ptr<node>& owner) {
    return std::shared_ptr<int>(owner, &owner->value);
}

inline std::weak_ptr<node> observe(const std::shared_ptr<node>& owner) noexcept {
    return owner;
}
