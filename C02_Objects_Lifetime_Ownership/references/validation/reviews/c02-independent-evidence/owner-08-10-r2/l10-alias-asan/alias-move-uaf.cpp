...
int main() {
    ...
    if (!p || p->value != 2 || p.use_count() != 1) {
        std::cerr << "move alias assignment produced wrong owner\n";
        return 2;
    }
    p.reset();
    const auto c = l10_support::counters();
    if (c.objects_alive != 0 || c.control_blocks_alive != 0) {
        std::cerr << "move alias assignment leaked: objects=" << c.objects_alive << " blocks=" << c.control_blocks_alive << "\n";
        return 3;
    }
    std::cout << "move alias assignment OK\n";
    return 0;
}
