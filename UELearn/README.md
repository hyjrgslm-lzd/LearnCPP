...
这套文档的目标受众，是已经完成 `Execution_Study` 或 `Ranges_Study`（或同等水平），现在想进入 UE 工程内核层的 C++ 工程师。
...
## 与 Execution_Study / Ranges_Study 的关系

这是三份正交的姊妹系列：

| 系列 | 核心问题 | 技术层 |
|---|---|---|
| `Ranges_Study` | **如何取值**——惰性值序列的产生与变换 | C++20 `<ranges>`，view、sentinel、CPO、borrowed_range |
| `Execution_Study` | **何时执行**——异步工作图的描述、组合与调度 | P2300 sender-receiver，stdexec，operation_state |
| `UELearn`（本系列） | **UE 引擎如何把这些组合成一个 editor + runtime**——Module 系统、反射、GC、多线程、渲染、网络 | UE5.7 内核层，UBT/UHT/UObject/RDG/NetDriver |

三份系列覆盖的不是同一层东西。`Ranges_Study` 回答"数据从哪里来"，`Execution_Study` 回答"工作在哪里跑"，`UELearn` 回答"一个工业级游戏引擎如何把这两件事以及更多的事，组织成一个可以被数百人团队协作的系统"。
...
1. 可运行代码（每题独立 Module，挂载在 `UELearn.uproject` 下）
2. 至少一张图（见下文"五张图触发规则"）
3. 5–10 行观察记录
4. 四固定问题书面回答
...
- **工程结构**：一练习一独立 Module。每题自己的 `.Build.cs` 声明依赖，宿主 `UELearn.uproject` 在 `Modules` 数组中统一注册。这种形态让你真实感受依赖裁剪、LoadingPhase 差异、循环依赖打破等 UBT 特有问题。
