...
源码阅读的目标不是记下私有函数名，而是验证一个具体问题：输入从哪里进入，状态在哪里改变，哪些出口表示失败，什么条件允许交付结果。本章固定MSVC STL提交`4edbc1d63a1ec156bed6dbf1727f413fa682abba`，对应输入SHA记录在[源码输入记录](../references/validation/upstream-stl-inputs.json)。安装版头文件另有指纹；不能把固定上游源码等同本机二进制，也不能因两个文件相似就推断所有实现一致。
