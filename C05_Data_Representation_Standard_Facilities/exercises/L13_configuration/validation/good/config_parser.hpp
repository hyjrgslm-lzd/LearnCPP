...
inline std::expected<c05::Config, c05::DataError> parse_config(std::string_view text)
{
    ...
    while (pos <= text.size()) {
        ...
        if (!trimmed.empty() && trimmed.front() != '#') {
            ...
            if (key == "package_file") {
                if (package) {
                    return std::unexpected(c05::DataError{c05::Errc::duplicate_field, key_off, c05::OffsetUnit::byte, "package_file", line});
                }
                ...
                if (zone) {
                    return std::unexpected(c05::DataError{c05::Errc::duplicate_field, key_off, c05::OffsetUnit::byte, "display_zone", line});
                }
