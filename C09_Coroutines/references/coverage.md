...
核对日期：2026-09-11。本表登记 37 个学习单元的入口、练习形态、Reference 与当前状态。入口接线不等于作业完成；最终运行和能力缺失由本机验证记录说明，不随课程源码提交。
...
- 普通练习的程序目标采用下表练习目录名，Reference通常在目录名后加`_reference`；Capstone5包含多个`mini_reference_*`组件目标，具体注册见其[CMakeLists](../exercises/Capstone5_mini_corolib/CMakeLists.txt)。实际构建/执行集合从本机验证记录回查，不把“已接线”当作全部平台验证通过。
...
每行列出的单元均已逐项登记；同类仅合并呈现，未删除原题。具体 Part → 操作 → 检查 → 解析在各题 README。`无需改` 表示无需重写已有正文，仍要参加所适用的验证。

| 单元 | 处置 | 下游任务与必要先修 | 证据／审查入口 |
|---|---|---|---|
| P1、P2 | 无需改，保留观察 | A3 future 通知、B1 递归 generator；先区分共享状态与协程状态 | 运行预备单元与对应 Reference |
| A1、A2 | 无需改，保留基础实现/观察 | B2、D 中使用 owner 与 yield/return；先理解 initial/final suspend | 运行 A 组 Reference |
| A3 | 学生检查补强 | 爬虫与 I/O 桥接；future 未就绪、完成位置、状态存活 | 运行学生检查与 Reference |
| B1、B2 | 学生检查补强 | 递归产出与顺序流水线；引用寿命、elements_of、值/异常传递 | 运行 B 组 Reference |
| B3 | 检查与 PASS 边界补强 | H/I；回调结果确实经过 await_resume，同步窗口另列解析/扩展 | 运行回调 awaiter 检查 |
| C1 | 检查与 PASS 边界补强 | 爬虫、RPC；协作取消与结果/异常，长计算不检查 token 的反例仍作观察 | 运行取消单元 |
| C2、C3 | 行为检查与教学补强 | G2、scope；全部子项先登记再释放，协程并发不等于不同 OS 线程 | 运行组合与 scope 检查 |
| Capstone1 | 代码/检查/错误路径补强 | generator 解析、task 组合、停止与排空；移动后错误 URL 在原失败上修复 | 运行项目 Reference |
| D1、D3 | 学生检查补强 | G/mini task；promise 完成与 continuation 转交，不把机器栈性质当标准保证 | 运行 promise 单元 |
| D2、E1、E2 | 无需改，保留观察与解析 | E3/H3；启动策略、查找、await_suspend 三返回形态 | 运行 D/E 观察与 Reference |
| E3、F1、F2 | 证据边界审计 | F3/mini 库；ready、状态/分配观察不能单靠耗时推断 HALO | 运行 frame/allocator 单元 |
| F3 | 实验与证据补全 | 分配消除判断；同契约基线、计数与无插桩 IR、独立进程样本分开 | 运行 F3 诊断；样本本地保存 |
| G1 | 明确有限单线程基线，衔接可注销等待者实现 | mini shared_task；多消费者、结果缓存与等待者寿命 | 运行 runtime 检查 |
| G2 | 学生 fan-out 检查补强 | mini when_all；登记/完成 barrier，不能用串行 drain 冒充组合 | 运行 when_all 检查 |
| G3 | 复杂样章与异常所有权补全 | 全部 task 消费；结果移动异常仍释放帧、final 通知期间状态保活 | 运行 sync_wait 检查 |
| H1、H2、H3 | Student 三链/探针/标准边界补强 | mini/应用；C10 sender 基础、操作状态、完成通道与 await 变换 | 运行 stdexec 相关单元 |
| I1、I2 | 真实 I/O 与 Student 检查补强 | RPC；C07 完成模型、buffer/handle 存活和关闭收束 | 运行平台 I/O 单元 |
| I3、I4 | 完整入口保留，检查骨架补强，运行未验证 | 固定 Folly/Cobalt 的 task/channel、executor 与所有权；当前缺依赖不缩减正文 | 依赖具备后运行 |
| I5 | 实际学生操作检查与版本边界补强 | cppcoro 对照阅读；TS 历史实现不冒充标准 API | 运行 cppcoro 单元 |
| J1、J2、J3 | 观察/诊断主体保留，实际复验 | 全课诊断；frame/闭包寿命、ABI 与 trace 的实现边界 | 运行 J 组观察 |
| Capstone4 | 契约、学生接口、逐阶段检查补全 | C05 字节、C07 I/O、H/I 生命周期；协议、pending、取消、重试、drain | 运行 RPC Reference |
| Capstone5 | 生命周期修复、学生检查与独立可做性补全 | D-G；scope 启动事务、shared waiter 注销、producer 寿命、一次性恢复 | 运行 mini 库 Reference 与学生检查 |
...
2026-09-09 之前的本机日志可能引用 `Coroutine_Study/`。重跑时映射为 `C09_Coroutines/` 并使用新的 build 目录；旧日志和指纹不提交为课程文档。
