...
function(coroutine_study_add_checker_variants name)
    ...
    foreach(variant good bad_constant)
        ...
        coroutine_study_add_ide_files(${name}_${variant})
        if(EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/checks/fixture.hpp")
            target_sources(${name}_${variant} PRIVATE "${CMAKE_CURRENT_SOURCE_DIR}/checks/fixture.hpp")
        endif()
        target_sources(${name}_${variant} PRIVATE "${CMAKE_CURRENT_SOURCE_DIR}/validation/${variant}/student.hpp")
...
function(coroutine_study_add_exercise name)
    ...
    coroutine_study_add_ide_files(${name})
    ...
    if(COROUTINE_STUDY_BUILD_REFERENCE AND ARG_REFERENCE_SOURCES)
        ...
        coroutine_study_add_ide_files(${name}_reference)
...
function(coroutine_study_add_ide_files target)
    target_sources(${target} PRIVATE "${COROUTINE_STUDY_EXERCISES_DIR}/include/coroutine_study/exercise_check.hpp")
    foreach(file README.md CMakeLists.txt)
        if(EXISTS "${CMAKE_CURRENT_SOURCE_DIR}/${file}")
            target_sources(${target} PRIVATE "${CMAKE_CURRENT_SOURCE_DIR}/${file}")
        endif()
    endforeach()
endfunction()
