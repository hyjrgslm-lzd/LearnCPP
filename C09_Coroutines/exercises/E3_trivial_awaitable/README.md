...
Visual Studio 中主启动目标是 `E3_trivial_awaitable`；Reference、Checks、Support 目标保留在同题分组中。学生编辑入口和本题 README/CMake 文件会显示在目标文件树里。

```powershell
cmake -S . -B build/vs -G "Visual Studio 18 2026" -A x64 -DCOROUTINE_STUDY_BUILD_REFERENCE=ON
cmake --build build/vs --config Debug --target E3_trivial_awaitable
```
