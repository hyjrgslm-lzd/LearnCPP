...
生成 Visual Studio 工程后，启动项目是 `L02_sync_io_student`。

本题是实现题。学习者只改 Student 入口；Reference、validation 和 checks 只用于对照与验证。
- Student 入口：`src/student/sync_io.hpp`。
- Checker 入口：`checks/sync_io_checks.cpp`。
- Reference 对照：`src/reference/sync_io.hpp`。
- validation/good 对照：`validation/good/sync_io.hpp`。
- validation/bad 反例：`validation/bad/sync_io.hpp`。

单题独立构建：从本目录运行 cmake -S . -B build/leaf -G "Visual Studio 18 2026" -A x64，然后 cmake --build build/leaf --config Debug --target L02_sync_io_student。
修改后先重建 `L02_sync_io_student`，再按课程 `BUILD_GUIDE.md` 运行对应检查。
