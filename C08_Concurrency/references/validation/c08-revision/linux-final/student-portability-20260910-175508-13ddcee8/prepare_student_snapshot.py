...
def skip_dir(path: Path) -> bool:
    return path.name.startswith(".") or path.name in EXCLUDE_DIRS or path.name.lower().startswith("build")


def copyable(path: Path) -> bool:
    if path.name in EXCLUDE_NAMES or path.suffix.lower() in EXCLUDE_EXTS:
        return False
    if any(part.startswith(".") or part in EXCLUDE_DIRS or part.lower().startswith("build") for part in path.relative_to(REPO).parts[:-1]):
        return False
    return True
