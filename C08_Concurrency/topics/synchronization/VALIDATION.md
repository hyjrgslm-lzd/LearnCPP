...
本作者所有编辑仍限 C08_Concurrency，使用 apply_patch，未提交 Git，未编辑任何 CMake、根 README、公共 benchmark 或覆盖/标准/质量索引。主线程新增的 CONCURRENCY_STUDY_TEST_STARTERS 接线已直接用于本次验证。ponytail 技能促使两种实现复用必要的契约检查；学生实现与答案独立，未建立通用测试框架。
...
路径相对 C08_Concurrency；SHA256 对应原始文件字节，不对本文做自哈希。
