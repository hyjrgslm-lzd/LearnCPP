...
Visual Studio 方案中，主入口目标是 `I3_rcu`，位于本题节点顶层；单题独立配置时它是启动目标。`I3_rcu_reference` 在 `Reference` 分组。
...
单题命令：从 `C08_Concurrency/exercises` 可独立配置：`cmake -S I3_rcu -B build/I3_rcu-ide -G "Visual Studio 18 2026" -A x64`，再构建 `I3_rcu` 或 `I3_rcu_reference`；要注册学生 CTest，按总 BUILD_GUIDE 的 student 模板加 `-DCONCURRENCY_STUDY_TEST_STARTERS=ON`。
