...
cl /nologo /std:c++23preview /EHsc /W4 /WX /utf-8 /O2 /DNDEBUG /IC08_Concurrency/exercises/include /FoC08_Concurrency/topics/reclamation/.build/R1_epoch_reclamation_student.obj /FeC08_Concurrency/topics/reclamation/.build/R1_epoch_reclamation_student.exe C08_Concurrency/exercises/R1_epoch_reclamation/main.cpp
./C08_Concurrency/topics/reclamation/.build/R1_epoch_reclamation_student.exe
$LASTEXITCODE
```
...
Visual Studio 方案中，主入口目标是 `R1_epoch_reclamation`，位于本题节点顶层；单题独立配置时它是启动目标。`R1_epoch_reclamation_reference` 在 `Reference` 分组。
...
单题命令：从 `C08_Concurrency/exercises` 可独立配置：`cmake -S R1_epoch_reclamation -B build/R1_epoch_reclamation-ide -G "Visual Studio 18 2026" -A x64`，再构建 `R1_epoch_reclamation` 或 `R1_epoch_reclamation_reference`；要注册学生 CTest，按总 BUILD_GUIDE 的 student 模板加 `-DCONCURRENCY_STUDY_TEST_STARTERS=ON`。
