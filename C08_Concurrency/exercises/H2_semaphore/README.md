...
Visual Studio 方案中，主入口目标是 `H2_semaphore`，位于本题节点顶层；单题独立配置时它是启动目标。`H2_semaphore_reference` 在 `Reference` 分组。

学生/观察入口：`main.cpp` 中 README 指定的 TODO。

Reference 与检查：`solution.cpp` 是 Reference/检查路径，只读对照。

单题命令：从 `C08_Concurrency/exercises` 可独立配置：`cmake -S H2_semaphore -B build/H2_semaphore-ide -G "Visual Studio 18 2026" -A x64`，再构建 `H2_semaphore` 或 `H2_semaphore_reference`；要注册学生 CTest，按总 BUILD_GUIDE 的 student 模板加 `-DCONCURRENCY_STUDY_TEST_STARTERS=ON`。
