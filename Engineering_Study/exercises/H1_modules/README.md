...
```powershell
cmake -S Engineering_Study/exercises/H1_modules -B Engineering_Study/exercises/build/learner-h1 -G Ninja -DCMAKE_BUILD_TYPE=Release
cmake --build Engineering_Study/exercises/build/learner-h1 --verbose
ctest --test-dir Engineering_Study/exercises/build/learner-h1 --output-on-failure
```
...
```powershell
cmake --build Engineering_Study/exercises/build/learner-h1 --target H1_hidden_type_negative --verbose
```
...
```powershell
$work = 'Engineering_Study/exercises/build/learner-h1-workspace'
if (Test-Path -LiteralPath $work) { throw '请选择尚不存在的工作目录' }
New-Item -ItemType Directory -Path $work | Out-Null
Copy-Item -LiteralPath Engineering_Study/exercises/H1_modules -Destination "$work/H1_modules" -Recurse
Copy-Item -LiteralPath Engineering_Study/exercises/cmake -Destination "$work/cmake" -Recurse
Copy-Item -LiteralPath Engineering_Study/exercises/include -Destination "$work/include" -Recurse
