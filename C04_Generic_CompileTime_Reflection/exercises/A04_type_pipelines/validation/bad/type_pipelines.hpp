...
template<class F, class E> struct transform_one<F, error_sig<E>> { using type = type_list<>; };
template<class F> struct transform_one<F, stopped_sig> { using type = type_list<>; };
