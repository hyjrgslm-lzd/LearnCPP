...
constexpr bool same_key(row const& left, row const& right) { return compare_key(left, right) == 0; }
constexpr bool same_key(row const& left, std::string_view right) { return compare_key(left, right) == 0; }
...
consteval auto make_table() {
    ...
    for (std::size_t i = Raw.size(); i != 0; --i) {
        ...
        for (std::size_t j = Raw.size(); j != i; --j) seen_later = seen_later || same_key(Raw[index], Raw[j - 1]);
        if (!seen_later) out.rows[write++] = Raw[index];
    }
