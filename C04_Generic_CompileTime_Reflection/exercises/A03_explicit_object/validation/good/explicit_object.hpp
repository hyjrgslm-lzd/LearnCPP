...
constexpr decltype(auto) forward_like(T&& value) noexcept {
    ...
    constexpr bool lvalue = std::is_lvalue_reference_v<Like>;
    constexpr bool constant = std::is_const_v<std::remove_reference_t<Like>>;
    if constexpr (lvalue && constant) return static_cast<const U&>(value);
    else if constexpr (lvalue) return static_cast<U&>(value);
    else if constexpr (constant) return static_cast<const U&&>(value);
...
public:
    ...
    T take(this slot&& self) noexcept { return std::move(self.value_); }
...
struct chain {
    ...
    int sum(this const chain& self) noexcept {
        auto visit = [](this auto const& visit, const chain& node) noexcept -> int {
            return node.value + (node.next ? visit(*node.next) : 0);
        };
        return visit(self);
    }
};
