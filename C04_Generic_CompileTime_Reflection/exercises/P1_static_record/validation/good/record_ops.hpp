    ...
    static constexpr void visit_fields(T&& value, F&& function)
        ...
        [&]<std::size_t... I>(std::index_sequence<I...>) {
            (static_cast<void>(std::invoke(function,
                std::get<I>(schema<std::remove_cvref_t<T>>::fields).name,
                std::forward<T>(value).*std::get<I>(schema<std::remove_cvref_t<T>>::fields).member)), ...);
        }(manual_detail::indices<T>{});
    }
};
