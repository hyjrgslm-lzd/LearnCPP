...
template<>
struct schema<::EmptyRecord> {
    static constexpr auto fields = std::tuple{};
};
template<>
struct schema<::Ticket> {
    static constexpr auto fields = std::tuple{
        field<"code", &::Ticket::code>{},
        field<"owner", &::Ticket::owner>{},
        field<"closed", &::Ticket::closed>{}
    };
};
}
...
template<class Record, c04_projection::fixed_string First, c04_projection::fixed_string Second, c04_projection::fixed_string Missing>
constexpr bool record_shape_ready() {
    if constexpr (!requires { typename c04_mp11::schema_map_t<Record>; }) {
        return false;
    } else {
        using map = c04_mp11::schema_map_t<Record>;
        return mp11::mp_is_map<map>::value
            && c04_mp11::field_count_v<Record> == 3
            && c04_mp11::keys_are_unique_v<Record>
            && c04_mp11::has_field_v<Record, First>
            && c04_mp11::has_field_v<Record, Second>
            && !c04_mp11::has_field_v<Record, Missing>;
    }
}

constexpr bool schema_shape_ready() {
    return record_shape_ready<c04_projection::Person, "id", "name", "missing">()
        && record_shape_ready<c04_projection::Order, "symbol", "quantity", "missing">()
        && record_shape_ready<Ticket, "code", "owner", "missing">();
}
...
void run_contract() {
    ...
    using namespace c04_projection;
    using person_map = c04_mp11::schema_map_t<Person>;
    using order_map = c04_mp11::schema_map_t<Order>;

    static_assert(mp11::mp_is_map<person_map>::value);
    static_assert(c04_mp11::field_count_v<Person> == 3);
    static_assert(c04_mp11::field_count_v<Order> == 3);
    static_assert(c04_mp11::field_count_v<Ticket> == 3);
    static_assert(c04_mp11::keys_are_unique_v<Person>);
    static_assert(c04_mp11::keys_are_unique_v<Ticket>);
    ...
    using product = c04_mp11::product_t<pair_names, string_fields, order_map>;
    ...
    id = 8;
    check(person.id == 8, "mp11 key lookup returns writable member reference");

    check(c04_mp11::visit_field_type<Person>(0, DescribeField{}) == "id:int",
        "runtime index 0 dispatches to id type");
    check(c04_mp11::visit_field_type<Person>(2, DescribeField{}) == "name:string",
        "runtime index 2 dispatches to name type");

    Ticket ticket{100, "ops", false};
    c04_mp11::get_by_key<"owner">(ticket) = "qa";
    check(ticket.owner == "qa", "independent record schema is not hard-coded to A02 records");
    check(c04_mp11::visit_field_type<Ticket>(2, DescribeField{}) == "closed:bool",
        "runtime dispatch works for an unfamiliar record");
    ...
    check(threw, "dynamic field dispatch must reject out-of-range indexes");
    static_assert(c04_mp11::field_count_v<EmptyRecord> == 0);
    static_assert(std::same_as<c04_mp11::find_field_t<EmptyRecord, "missing">, void>);
    int empty_calls = 0;
    bool empty_threw = false;
    try {
        c04_mp11::visit_field_type<EmptyRecord>(0, [&]<class Pair>() { ++empty_calls; });
    } catch (const std::out_of_range&) {
        empty_threw = true;
    }
    check(empty_threw && empty_calls == 0,
        "empty schema dispatch rejects without instantiating or invoking a field");
    }
}
}

int main() {
    constexpr bool shape_ready = schema_shape_ready();
    check(shape_ready, "schema_map_t<Person> must expose the Person fields before the full Mp11 contract can run");
    run_contract<shape_ready>();
}
