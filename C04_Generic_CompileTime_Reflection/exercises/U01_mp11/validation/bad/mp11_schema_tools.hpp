...
struct to_pair<c04_projection::field<Name, Pointer>> {
...
using schema_map_t = typename bad_detail::tuple_to_map<
...
decltype(auto) visit_field_type(std::size_t index, Visitor&& visitor) {
    ...
    const std::size_t unchecked = index < count ? index : 0;
    if constexpr (count == 0) {
        throw std::out_of_range("field index out of range");
    } else {
        return mp11::mp_with_index<count>(unchecked, [&]<class I>(I) -> decltype(auto) {
