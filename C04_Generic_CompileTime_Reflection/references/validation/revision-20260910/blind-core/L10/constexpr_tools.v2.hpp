...
struct decimal_parser<Text, Index, Accumulator, false> {
    ...
    static constexpr int digit = ch - '0';
    static_assert(Accumulator <= (std::numeric_limits<int>::max() - digit) / 10, "decimal_value exceeds int range");

    static constexpr int value = decimal_parser<Text, Index + 1, Accumulator * 10 + digit>::value;
};
