...
模式：`full`。记录时间：`2026-09-09T16:49:59.823440+00:00`。随机种子：`40412025`。
...
- 计时 clock：`time.monotonic()` implementation `GetTickCount64()`，resolution `0.015625`；`time.perf_counter()` implementation `QueryPerformanceCounter()`，resolution `1e-07`。本实验保持 C01 runner 的 `monotonic` 协议。
...
- 样本值保留 4 位小数用于复算展示；本机 `time.monotonic()` resolution 决定了可分辨边界，不能从 1ms 级差异推断收益。
...
| 项数 | 版本 | 有效样本秒 | 中位数 | 范围 |
|---:|---|---|---:|---|
| 32 | recursive | 0.0940, 0.0940, 0.0930, 0.1100, 0.1090 | 0.0940 | 0.0930-0.1100 |
| 32 | fold | 0.0940, 0.0940, 0.0940, 0.1250, 0.0940 | 0.0940 | 0.0940-0.1250 |
| 128 | recursive | 0.1090, 0.1410, 0.1090, 0.1100, 0.1250 | 0.1100 | 0.1090-0.1410 |
| 128 | fold | 0.0940, 0.1100, 0.0940, 0.1250, 0.1090 | 0.1090 | 0.0940-0.1250 |
| 256 | recursive | 0.1570, 0.1400, 0.1720, 0.1410, 0.1720 | 0.1570 | 0.1400-0.1720 |
| 256 | fold | 0.0940, 0.1250, 0.0930, 0.1090, 0.1100 | 0.1090 | 0.0930-0.1250 |

## 类型查询 trace 定位

| 项数 | 版本 | Total Frontend us | Total InstantiateClass count | Total InstantiateFunction count |
|---:|---|---:|---:|---:|
| 32 | recursive | 23173 | 6 | 4 |
| 32 | fold | 22147 | 3 | 4 |
| 128 | recursive | 37588 | 6 | 4 |
| 128 | fold | 22193 | 3 | 4 |
| 256 | recursive | 79812 | 6 | 4 |
| 256 | fold | 25670 | 3 | 4 |
...
| 版本 | 有效样本秒 | 中位数 | 范围 |
|---|---|---:|---|
| implicit | 0.4220, 0.3590, 0.3440, 0.3750, 0.4220 | 0.3750 | 0.3440-0.4220 |
| explicit | 0.4390, 0.4380, 0.4220, 0.4370, 0.4530 | 0.4380 | 0.4220-0.4530 |
