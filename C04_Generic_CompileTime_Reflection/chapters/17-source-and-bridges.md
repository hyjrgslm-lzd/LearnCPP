...
本轮固定的本机STL输入为MSVC工具集14.51.36231、STL更新202604。`type_traits`和`xutility`的文件指纹另存于[source-inputs.json](../references/validation/source-inputs.json)。这些是安装头文件的真实版本，不冒称某个上游Git提交；后续机器行号变化时，先匹配版本/哈希和符号，再跟随调用。
