...
int box_init(ExportBox* self, PyObject* args, PyObject*) {
  PyObject* bytes{};
  if (!PyArg_ParseTuple(args, "O", &bytes)) return -1;
  if (!PyBytes_CheckExact(bytes)) {
    PyErr_SetString(PyExc_TypeError, "ExportBox requires exact bytes");
    return -1;
  }
  if (self->exports != 0) {
    PyErr_SetString(PyExc_BufferError, "cannot reinitialize while exported");
    return -1;
  }
  PyObject* next = Py_NewRef(bytes);
...
PyObject* transform_view(PyObject*, PyObject* obj) {
  ...
  auto* src = static_cast<unsigned char*>(view.buf);
  ...
  for (Py_ssize_t i = 0; i < view.len; ++i) {
    ...
    dst[i] = static_cast<unsigned char>((byte >= 'a' && byte <= 'z') ? byte - 32 : byte);
  }
  return result;
}
