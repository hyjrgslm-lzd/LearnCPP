...
public:
  RewriteCallback(clang::tooling::Replacements& replacements,
  ...
  void run(const MatchFinder::MatchResult& result) override {
    ...
    if (!is_rewritable_user_loc(sm, loc) || (ref != nullptr && has_template_ancestor(ast, *ref)) ||
        (decl != nullptr && has_template_ancestor(ast, *decl))) {
      auto [file, line] = location_text(sm, loc);
      findings_.push_back({file, line, "refuse macro/template/system-header rewrite"});
      return;
    }
    std::string error;
    auto file = sm.getFilename(sm.getSpellingLoc(loc));
    auto offset = sm.getFileOffset(sm.getSpellingLoc(loc));
    if (!add_replacement(replacements_, seen_, clang::tooling::Replacement(file, offset, old_symbol_.size(), new_symbol_), error)) {
      auto [where, line] = location_text(sm, loc);
      findings_.push_back({where, line, "replacement conflict: " + error});
    }
  }
...
void register_rewrite_matchers(MatchFinder& finder,
                               ...
                               std::string old_symbol,
                               std::string new_symbol) {
  static std::vector<std::unique_ptr<RewriteCallback>> callbacks;
  callbacks.push_back(std::make_unique<RewriteCallback>(replacements, findings, old_symbol, new_symbol));
