...
inline void close_native(NativeBox* box);

inline int close_box(lua_State* L) {
  auto* box = static_cast<NativeBox*>(lua_touserdata(L, 1));
  close_native(box);
  return 0;
}

inline void close_native(NativeBox* box) {
  if (box != nullptr && !box->closed) {
    box->closed = true;
    ++*box->closes;
  }
}
...
inline GcResult exercise_userdata() {
  ...
  lua_pushcfunction(L, [](lua_State* L) -> int {
    ...
    luaL_newmetatable(L, "c18.NativeBox");
    lua_pushcfunction(L, gc_box);
    lua_setfield(L, -2, "__gc");
    auto* box = static_cast<NativeBox*>(lua_newuserdatauv(L, sizeof(NativeBox), 0));
    *box = NativeBox{&args->closes, &args->gc_calls, false};
    lua_pushvalue(L, -2);
