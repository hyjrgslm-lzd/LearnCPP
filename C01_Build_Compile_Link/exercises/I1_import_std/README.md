...
```powershell
cmake -S C01_Build_Compile_Link/exercises/I1_import_std -B C01_Build_Compile_Link/exercises/build/learner-i1 -G Ninja -DCMAKE_BUILD_TYPE=Release
cmake --build C01_Build_Compile_Link/exercises/build/learner-i1 --verbose
ctest --test-dir C01_Build_Compile_Link/exercises/build/learner-i1 --output-on-failure
```
...
```powershell
$source = (Resolve-Path C01_Build_Compile_Link/exercises/I1_import_std/reference/main.cpp).Path
$stdSource = Join-Path $env:VCToolsInstallDir 'modules/std.ixx'
$out = 'C01_Build_Compile_Link/exercises/build/learner-i1-direct'
...
**解析：** 能力发现发生在语言启用过程中，后设置gate不能当作首次正确启用。不同工具版本的诊断文字可能不同，必须核对缺的是gate、metadata、标准级别、生成器还是链接输入，不能将任意失败统一写成“不支持modules”。标准库模块与 C++20 named modules 要分别验证，未运行的平台不能继承 Windows 结果。
