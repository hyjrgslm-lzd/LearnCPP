...
这张表登记教学责任和练习入口。阅读正文、运行观察、完成实现题是不同的动作；对应练习 README 给出逐 Part 作业、独立编辑位置和解析。覆盖表不记录本机验证日志、测量结果或交付过程状态。
...
1. **Concurrency 的 verify-core 为什么不等于完整库能力？** A1/F1/F2/G1必须使读者能区分配置、实例化、链接、运行、未请求和真实SKIP，再进入并发课的native probes。原并发算法和性能正文保留在[C08主课](../../C08_Concurrency/README.md)。
2. **Coroutine J2为什么不应跨模块裸传协程帧？** C2/D1/E1必须先解释符号只解决名字、ABI还约束布局与运行时、对象销毁属于哪个模块，然后再看协程额外的帧存活责任。具体帧协议仍由[C09](../../C09_Coroutines/README.md)主讲。
...
检查器只证明具体运行条件，不替代教学理解。C01不代替C02完整对象模型、C04模板进阶、C07系统I/O、C13完整性能数值课程或C18生产插件/语言绑定。
